...
export default function Result() {
  ...
  const textStyle = { color: C('muted-foreground', 0.8), lineHeight: '1.8', fontSize: '14px' };

  // 架构师分析渲染
  function renderArchitect(arch) {
    if (!arch) return <div className="glass-card p-8"><p style={textStyle}>暂无架构分析数据</p></div>;
    return (
      <div className="space-y-4 anim-fade-in">
        <div className="glass-card p-6" style={{ borderColor: C('primary', 0.2) }}>
          ...
          <p className="text-sm mb-3" style={textStyle}>{arch.coreConflict?.description || ''}</p>
  ...
  function renderCharacterDesign(cd) {
    if (!cd) return <div className="glass-card p-8"><p style={textStyle}>暂无人物设计数据</p></div>;
    return (
      <div className="space-y-4 anim-fade-in">
        {cd.protagonist && (
          <div className="glass-card p-6" style={{ borderColor: C('primary', 0.2) }}>
          ...
          <div className="glass-card p-6">
            ...
            {cd.supporting.map((s, i) => (
              <div key={i} className="mb-4 last:mb-0 p-3 rounded" style={{ background: C('card') }}>
                ...
                <p className="text-xs" style={textStyle}>{s.traits?.join(' · ')} | {s.role}</p>
  ...
  function renderShowrunner(sr) {
    if (!sr) return <div className="glass-card p-8"><p style={textStyle}>暂无总编审数据</p></div>;
      ...
      <div className="space-y-4 anim-fade-in">
        ...
        {sr.marketAnalysis && (
          <div className="glass-card p-6">
            ...
            <p className="text-xs" style={textStyle}>{sr.marketAnalysis.differentiation}</p>
          </div>
        )}
              ...
              return (
                <div key={platform} className="mb-4 last:mb-0 p-4 rounded-lg" style={{ background: C('primary', 0.03) }}>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div>
                      <span style={{ color: '#2ecc71' }}>✓ {pa.strength}</span>
                    </div>
                    <div>
                      <span style={{ color: '#e74c3c' }}>✗ {pa.weakness}</span>
                    </div>
                  </div>
                  {pa.adjustment && (
                    <p className="text-xs mt-2" style={{ color: C('muted-foreground', 0.6) }}>💡 {pa.adjustment}</p>
                  )}
                </div>
              );
            })}
        ...
        {sr.coreIssue?.hasIssue && (
          <div className="glass-card p-6" style={{ borderColor: '#e74c3c' }}>
            ...
            <p className="text-xs" style={textStyle}>修复方案：{sr.coreIssue.fix}</p>
          </div>
        )}
      </div>
    );
      ...
      case 'synopsis':
        return (
          <div className="space-y-5 anim-fade-in">
            ...
            <div className="glass-card p-8"><pre className="whitespace-pre-wrap font-sans" style={textStyle}>{result.synopsis}</pre></div>
          </div>
        );
      case 'characters':
        return <div className="glass-card p-8 anim-fade-in"><pre className="whitespace-pre-wrap font-sans" style={textStyle}>{result.characters}</pre></div>;
      case 'outline':
        return <div className="glass-card p-8 anim-fade-in"><pre className="whitespace-pre-wrap font-sans" style={textStyle}>{result.episodeOutline}</pre></div>;
      case 'script': {
          ...
          <div className="space-y-4 anim-fade-in">
            ...
            <div className="glass-card p-8 relative" style={{ borderColor: isLocked ? 'hsl(var(--muted) / 0.3)' : C('primary', 0.12) }}>
              ...
              {isLocked ? (
                ...
                <pre className="whitespace-pre-wrap font-sans" style={{ color: C('foreground'), lineHeight: '1.9', fontSize: '14px' }}>
        ...
        return (
          <div className="space-y-5 anim-fade-in">
            <div className="glass-card p-8 text-center">
              <div className="text-7xl font-black mb-3 text-shimmer">{grade}</div>
              <p className="text-lg font-bold" style={{ color: C('foreground') }}>
                综合评分 {totalScore} / 80
                <span style={{ color: C('muted-foreground', 0.6), fontSize: '14px' }}>（加权 {weightedScore}）</span>
              </p>
              ...
              <div className="glass-card p-6" style={{ borderColor: C('primary', 0.12) }}>
                ...
                {ds.total != null && (
                  <div className="text-center mb-3">
                    ...
                    <span className={`tag ml-2 text-[10px] ${ds.grade === '优' ? 'tag-green' : ds.grade === '良' ? 'tag-yellow' : 'tag-red'}`}
                      style={{
                        background: ds.grade === '优' ? 'rgba(46,204,113,0.1)' : ds.grade === '良' ? 'rgba(243,156,18,0.1)' : 'rgba(231,76,60,0.1)',
                        color: ds.grade === '优' ? '#2ecc71' : ds.grade === '良' ? '#f39c12' : '#e74c3c',
                      }}>{ds.grade}</span>
                  </div>
                )}
            ...
            <div className="glass-card p-5 text-xs" style={{ color: C('muted-foreground', 0.7), lineHeight: '1.8' }}>
              <span style={{ color: C('muted-foreground'), fontWeight: 700 }}>评级标准：</span>
              S+（70-80）可直接拍摄上线 · A（60-69）微调后可上线 · B（50-59）需较大修改 · C（&lt;50）建议重构
            </div>
          </div>
        );
      }
      case 'notes':
        return <div className="glass-card p-8 anim-fade-in"><pre className="whitespace-pre-wrap font-sans" style={textStyle}>{result.notes}</pre></div>;
      case 'commercial':
        return <div className="glass-card p-8 anim-fade-in"><pre className="whitespace-pre-wrap font-sans" style={textStyle}>{result.commercial}</pre></div>;
      case 'architect':
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-6" style={{ background: C('background'), paddingTop: '6vh' }}>
      <div className="w-full max-w-xl">
        <div className="text-center mb-10 anim-fade-up">
          <div className="text-5xl mb-5">✦</div>
          <h2 className="text-2xl font-black mb-3" style={{ color: C('foreground') }}>
            你的<span style={{ color: C('primary') }}>传奇</span>已生成
          </h2>
        ...
        <div className="flex flex-wrap gap-2 mb-8 justify-center anim-fade-in">
          {tabs.filter(t => !t.requires || result[t.requires]).map((t) => (
            <button
              ...
              style={{
                background: activeTab === t.key ? C('primary', 0.1) : C('card'),
                color: activeTab === t.key ? C('primary') : C('muted-foreground', 0.7),
                border: activeTab === t.key ? `1px solid ${C('primary', 0.25)}` : `1px solid ${C('border')}`,
              }}
            >
              {t.label}
            </button>
          ...
          <div className="glass-card p-6" style={{ borderColor: C('primary', 0.15) }}>
            ...
            <div className="grid grid-cols-3 gap-3 mt-5">
              {[
                ...
                <div key={i} className="p-3 rounded-lg text-center cursor-pointer glass-card-hover"
        ...
        <div className="flex flex-wrap gap-2 justify-center pb-10 anim-fade-up">
          ...
          <button onClick={() => exportCustomerDOCX(result, scriptCount)} className="btn-secondary text-xs" style={{ background: C('card'), border: `1px solid ${C('border')}` }}><span>📄 客户预览版 DOCX</span></button>
      ...
      {showPricing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6" style={{ background: 'hsl(var(--background) / 0.85)', backdropFilter: 'blur(8px)' }} onClick={() => setShowPricing(false)}>
          <div className="w-full max-w-md max-h-[85vh] overflow-y-auto glass-card p-8 anim-fade-up" onClick={(e) => e.stopPropagation()} style={{ borderColor: C('primary', 0.25) }}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-black" style={{ color: C('foreground') }}>创作套餐</h3>
              <button onClick={() => setShowPricing(false)} className="text-2xl leading-none" style={{ color: C('muted-foreground', 0.6) }}>✕</button>
            </div>
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i}
                  ...
                  style={p.popular ? { borderColor: C('primary', 0.35), boxShadow: '0 0 24px hsl(var(--primary) / 0.08)' } : {}}
                ...
                >
                  {p.popular && (
                    <div className="absolute -top-2.5 left-1/2 -translate-x-1/2 px-3 py-0.5 rounded-full text-xs font-bold"
                      style={{ background: C('primary'), color: C('primary-foreground') }}>推荐</div>
                  )}
                  ...
                  <div className="text-xs leading-relaxed mt-2" style={{ color: C('muted-foreground', 0.7), whiteSpace: 'pre-line' }}>{p.desc}</div>
                </div>
              ))}
            </div>
