...
// HSL variable helper: C('primary') → hsl(var(--primary)), C('muted-foreground', 0.5) → hsl(var(--muted-foreground) / 0.5)
  ...
  return (
    <div className="min-h-screen flex flex-col items-center px-6" style={{ background: C('background'), paddingTop: '8vh' }}>
      <div className="w-full max-w-lg">
        ...
        <h2 className="text-2xl font-black text-center mb-3" style={{ color: C('foreground') }}>选择创作方向</h2>
        <p className="text-sm text-center mb-12" style={{ color: C('muted-foreground', 0.6) }}>同一位企业家，三种完全不同的剧本玩法</p>
          ...
          {entrepreneurDirections.map((dir) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-6 rounded-xl transition-all duration-300 ${selected ? 'card-active' : 'glass-card-hover'}`}
              >
        ...
        <button
          ...
          style={!selectedDirection ? { background: C('muted'), color: C('muted-foreground', 0.5), boxShadow: 'none', cursor: 'not-allowed' } : {}}
        >
          <span>下一步：回答问题 →</span>
        </button>
        <button onClick={() => setStep('landing')} className="btn-secondary w-full mt-4">← 返回首页</button>
